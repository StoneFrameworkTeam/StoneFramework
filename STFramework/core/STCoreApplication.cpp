...
class STCoreApplication::PrivateData
{
public:
    class EventPackage
    {
    ...
    STExitReason waiteAndDealEvent()
    {
        ...
        while (1) {
            while (m_eventList.size() != 0) {
                m_eventMutex.lock();
                const PrivateData::EventPackage e = m_eventList.front();
...
STExitReason STCoreApplication::exec()
{
    STExitReason ret = STExitReason_Normal;
...
void STCoreApplication::exit(STExitReason reason)
{
...
void STCoreApplication::postEvent(STEventCarrier e, STObject* receiver)
{
    PrivateData::EventPackage eventPkg(e, receiver);
    m_data->postEventPkg(eventPkg);
}

bool STCoreApplication::sendEvent(STEventCarrier e, STObject* receiver)
{
    PrivateData::EventPackage eventPkg(e, receiver);
...
void STCoreApplication::addEventFilter(STObject* filter, STEvent e)
{
    m_data->addEventFilter(filter, e);
}

void STCoreApplication::removeEventFilter(STObject* filter, STEvent e)
{
